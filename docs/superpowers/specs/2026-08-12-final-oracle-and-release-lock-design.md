...
`Cargo.lock` is generated by official Cargo against an index constrained to
packages declaring compatibility with Rust 1.85 or older, then committed. The
pinned Rust 1.85 CI metadata gate is the release authority for final lockfile
compatibility because Windows Application Control blocks local Cargo 1.85 and
the remote stable service cannot install an older toolchain without network.
