...
def size(tree: Path) -> dict:
    ...
    return {"files": len(files), "lines": lines, "bytes": sum(p.stat().st_size for p in files)}
...
def dustmite(work: Path, case: Path) -> dict:
    ...
    environment = dict(os.environ, BENCHMARK_COUNTER=str(counter), BENCHMARK_POLARITY="interesting")
...
def reprocut(work: Path, case: Path) -> dict:
    ...
    counter = home / "counter"
    counter.write_bytes(b"")
    environment = dict(os.environ, BENCHMARK_COUNTER=str(counter), BENCHMARK_POLARITY="failing")
    start = time.monotonic()
    with (home / "tool.log").open("wb") as log:
        completed = subprocess.run(
            [
                ...
                "regex",
                "--failure-regex",
                "REPO-ARENA: files were modified by this hook",
                "--max-duration-secs",
                str(BUDGET),
                "--",
                str(ORACLE),
...
def main() -> int:
    ...
    print(f"{'original':<11}{original['files']:>8,}{original['lines']:>9,}{original['bytes']:>11,}")
    for row in rows:
        if not row.get("available"):
            print(f"{row['tool']:<11}{'not installed':>28}")
        elif not row.get("produced_output"):
            print(
                f"{row['tool']:<11}{'no output':>28}{row['oracle_calls']:>9,}{row['seconds']:>9.1f}"
            )
        else:
            print(
                f"{row['tool']:<11}{row['files']:>8,}{row['lines']:>9,}"
